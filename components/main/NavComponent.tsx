...
import {
  ...
  Nav_Title_MKS,
  Nav_Title_Sistemas,
} from "@/styles/main/nav";
import { useDispatch } from "react-redux";

export default function NavComponent() {
  const quantity: number = useAppSelector((state) => state.cart.quantity);
  ...
  return (
    <Nav_Container>
      <Nav_Content>
        <Nav_Title_Container>
          <Nav_Title_MKS>MKS</Nav_Title_MKS>
          <Nav_Title_Sistemas>Sistemas</Nav_Title_Sistemas>
        </Nav_Title_Container>
        <Nav_Cart_Button id="button" onClick={onHandleCart}>
          <Nav_Cart_SVG
            viewBox="0 0 20 18"
            fill="none"
